import React from "react";
import { BsPlusCircle } from "react-icons/bs";

const Home = (props) => {
  // console.warn("props", props);
  return (
    <div className="row position-relative ">
      <h1>Home Component</h1>
      <div className="col-md-6 col-10 d-flex border rounded ms-5 p-2 py-3">
        <div>
          <div className="">
            <img
              className="img-fluid"
              style={{ width: 250 }}
              src="https://www.pngmart.com/files/15/Apple-iPhone-11-PNG-Clipart.png"
              alt="mobile_image"
            />
          </div>
          <div className="">
            <span>I-PhonePrice</span>
            <br />
            <span>$10000.00</span>
          </div>
          <div>
            <button
              className="bg-danger text-white border-none p-1 mt-3"
              onClick={() =>
                props.addCartHandler({
                  price: "10000",
                  name: "IPhone 11 pro",
                })
              }
            >
              Add To Cart
            </button>
          </div>
        </div>
      </div>
      <div className="position-relative position-absolute">
        <img
          className="img-fluid position-absolute top-0 end-0 me-5"
          style={{ width: 50 }}
          src="data:image/png;base64,"
          alt="cart-img"
        />
        <BsPlusCircle
          className="position-absolute fs-3 bg-white top-0 end-0 me-5"
          style={{ marginTop: -15 }}
        />
      </div>
    </div>
  );
};

export default Home;
